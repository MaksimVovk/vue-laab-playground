<template>
  <div :class="['vue-lab__lab-button', ...classes]" @click="handleClick">
    <slot />
  </div>
</template>
...
const classes = computed(() => [
  `vue-lab__lab-button_size-${props.size}`,
  `vue-lab__lab-button_palette-${props.palette}`,
])
...
.vue-lab__lab-button {
  ...
  @each $name, $size in $sizes {
    &_size-#{$name} {
      ...
      @include button-size(get($size, padding), get($size, height), get($size, border-radius));
    }
  }
}
</style>
